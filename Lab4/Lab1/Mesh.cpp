...
{
	for (int j = 0; j < model.positions.size(); j++)
	{
		if (model.positions[j].y < lowestY)
		{
			lowestY = model.positions[j].y;
		}
		if (model.positions[j].y > highestY)
		{
			highestY = model.positions[j].y;
		}
		if (model.positions[j].x < lowestX)
		{
			lowestX = model.positions[j].x;
		}
		if (model.positions[j].x > highestX)
		{
			highestX = model.positions[j].x;
		}
	}
